...
- `/admin/` — административная панель (временный интерфейс для Контролёров/
  Юристов и Администраторов до появления рабочих мест из ТЗ 4.1).
...
- **Smart Search — сам поиск** (ТЗ 4.4.1, `apps/search_ocr/search.py`) —
  ...
  FTS(ocr_body)*0.2`. Web GUI на `/` (домашняя страница). `ocr_body`
  заполняется асинхронно конвейером OCR (см. Этап 3 ниже) — до его
  завершения по конкретному документу поле пусто. Честные границы (нет
  постраничной навигации, нет External API-эндпоинта, индекс вычисляется
  на лету без персистентного `SearchVectorField`) — STACK.md.
...
- **Конвейер OCR** (`apps/documents/ocr.py`, `apps/documents/tasks.py`) —
  распознавание скана НРД запускается асинхронно (Celery + Redis,
  `run_ocr_for_document`) при первой загрузке/замене `files_original`
  (`NormativeDocument.save()`, через `transaction.on_commit()`, чтобы
  воркер не читал ещё не закоммиченный файл). Растеризация PDF —
  `pdf2image`/poppler, распознавание — `pytesseract`/Tesseract (язык —
  только русский, `settings.OCR_LANGUAGE`). Результат — `ocr_body`
  (сплошной текст, участвует в `FTS(ocr_body)` формулы Smart Search) и
  `ocr_confidence` (средняя уверенность по словам, шкала 0-100 —
  собственная шкала Tesseract). Отказ распознавания — до 3 повторов
  (`max_retries=3`), затем одна запись `DOCUMENT_OCR_FAILED` в WORM-аудит
  (успех — `DOCUMENT_OCR_COMPLETED`). Честные границы: нет UI прогресса/
  очереди распознавания, нет ретроактивного перезапуска для уже
  загруженных ранее документов (задача ставится только на изменение
  `files_original`), нет отдельного result backend — воркер запускается
  локально (`celery -A config worker -l info`), в `docker-compose.yml`
  для него не заведён отдельный контейнер (само приложение там не
  контейнеризовано, см. «Локальный запуск» выше). Подробности — STACK.md.

## Дальше по плану

Анти-фрод, HA-кластер и DR-регламенты — оставшаяся часть Этапов 2–4 плана
работ. Открытые вопросы, требующие решения Заказчика,
