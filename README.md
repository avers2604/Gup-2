...
- исправление ошибочной публикации двумя разными путями — откат в черновик и
  аннулирование (статус «Аннулирован» ≠ «Утратил силу»): оба только Администратору
  и с обязательным основанием;
- история действий по документу на его карточке — кто опубликовал, кто отменил,
  кто менял связи и почему;
...
- сводные редакции: документ и все действующие изменения к нему в одном месте
  (перечень собирается по графу связей, а не по статусу карточки);
- рабочее место вычитки OCR: разбор очереди документов, распознанных ниже
  порога уверенности, с записью правки в WORM-журнал;
- выгрузка журнала аудита в CSV и запись в журнал самого факта выгрузки;
- фиксация в WORM-журнале выдачи файла документа с грифом «ДСП».
...
| `/documents/consolidated/` | сводные редакции: документы с действующими изменениями |
| `/documents/consolidated/<uuid>/` | сводка по одному документу |
| `/documents/ocr-review/` | очередь ручной вычитки распознанного текста |
| `/documents/ocr-review/<uuid>/` | вычитка текста одного документа |
...
| `/audit/export/` | выгрузка журнала в CSV (пишет `AUDIT_LOG_EXPORTED`) |
