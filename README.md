...
- **Smart Search — сам поиск** (ТЗ 4.4.1, `apps/search_ocr/search.py`) —
  ...
  FTS(ocr_body)*0.2`. Web GUI на `/` (домашняя страница). Честные границы
  (`ocr_body` всегда пусто — конвейера OCR ещё нет, нет постраничной
  навигации, нет External API-эндпоинта, индекс вычисляется на лету без
