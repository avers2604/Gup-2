...
- **Smart Search — сам поиск** (ТЗ 4.4.1, `apps/search_ocr/search.py`) —
  ...
  веса TH-06, разрешение неоднозначных аббревиатур через `ThesaurusAmbiguity`)
  и полнотекстовый поиск по карточкам НРД (`search_documents`, Postgres
  full-text, `config=russian`) с гибридным ранжированием. Web GUI на `/`
  (домашняя страница). Честные границы (формула ранжирования — не
  дословно ТЗ 4.4.1, нет постраничной навигации, нет External API-эндпоинта,
  индекс вычисляется на лету без персистентного `SearchVectorField`) —
  STACK.md.
...
Конвейер OCR, анти-фрод, HA-кластер и DR-регламенты — оставшаяся часть
Этапов 2–4 плана работ. Открытые вопросы, требующие решения Заказчика,
