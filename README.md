...
- `/documents/` — реестр НРД (фильтры, постраничная навигация),
  `/documents/<uuid>/` — карточка документа (ТЗ 4.1). Запись:
  `/documents/new/` (регистрация), `/documents/<uuid>/edit/` (правка
  черновика), `/documents/<uuid>/status/` (смена статуса).
- `/admin/` — административная панель. Рабочие места из ТЗ 4.1 закрывают
  основную работу с НРД; в админке остаются операции, до которых Web GUI
  ещё не дошёл (связи версионности, справочники, бланки).
...
- **Рабочее место «Реестр и карточка НРД»** (ТЗ 4.1, 4.2,
  ...
  **требует подтверждения Заказчиком**. Ключевое: допуск «ДСП» —
  ...
  карточек, иначе гриф обходился бы сбоку. Запись добавлена следующей
  партией (см. ниже). В навигации временно нет пункта «Бланки» — он
  вернётся вместе с рабочим местом банка бланков. Подробности — STACK.md.
- **Запись НРД: регистрация, правка, смена статуса** (ТЗ 4.1, 4.2.2,
  4.2.3, `apps/documents/services.py`, `transitions.py`) — рабочее место
  НРД перестало быть «только чтением». Создание всегда даёт черновик;
  правка разрешена только черновику (документ в силе изменяется новой
  редакцией со связью версионности); смена статуса — отдельное действие
  Контролёра/Юриста с записью в WORM-журнал и необязательным
  основанием, которое туда же и попадает. Загрузка `files_original`/
  `files_editable` идёт через уже существующий контур проверок
  (антивирус + макросы, fail-closed) и запускает распознавание.
  **Закрыт пробел ТЗ 4.2.3: SCD-2 история статусов не велась вообще** —
  модель, exclusion constraint и вывод в карточке были с Этапа 1, а
  писать в таблицу было некому, и у любого документа история оставалась
  пустой навсегда. Теперь срезы закрываются и открываются одной и той же
  отметкой времени (`[t0, t1)` + `[t1, NULL)`, без дыр) на любом пути
  записи, включая админку и импорт. Допустимые переходы вынесены в
  `transitions.py` и, как и матрица прав, **выведены, а не процитированы**
  — **требуют подтверждения Заказчиком**. Публикация проверяет граф
  связей на ацикличность — дословное требование ТЗ 4.2.2. Попутно
  найдено живой проверкой в браузере: `django.contrib.messages` был
  настроен, но ни один шаблон его не выводил (сообщения молча терялись).
  Честные границы: связи версионности по-прежнему заводятся только через
  админку; правка полей черновика не пишется в журнал (в отличие от
  смены статуса и категории хранения). Подробности — STACK.md.
