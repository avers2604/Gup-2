...
`LoginFailure` не является WORM-журналом и не предназначен для бессрочного хранения. Для самого lockout нужны только последние 15 минут. Очистка устаревшей operational history должна выполняться отдельной эксплуатационной задачей; удаление этих строк не затрагивает неизменяемый `AuditLog`, который остаётся источником аудиторских доказательств.
