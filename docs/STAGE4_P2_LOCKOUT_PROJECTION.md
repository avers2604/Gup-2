...
Порог задаётся через `IAM_IP_LOCKOUT_MAX_ATTEMPTS` (default `20`). Окно в этой партии остаётся 15 минут. Это позволяет менять threshold между стендом/площадками без изменения кода.
