...
восстановление на момент времени через пять смен timeline подряд) и оба
drill'а Celery/Redis `kill -9` (Redis — с включённым `appendonly`; Celery
worker — повторно, уже с исправленным Ф-8) — ни одной потери, ни одного
...
юнит-тестах на моках — только на реальном Redis. Повторный прогон того же
сценария с исправлением — PASS: те же 4 пробы корректно redelivered,
все 20 завершены ровно по одному разу.
