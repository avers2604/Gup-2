...
if not lines:
    raise SystemExit("reindex command produced no JSON measurement")
...
PY
    if (( rc == 0 )); then
      checkpoint cold-reindex-10000-passed
      echo "Cold reindex PASS; measurement=$out"
    else
      checkpoint cold-reindex-10000-failed
  ...
  minio-hash-500|minio-hash)
    ...
    MINIO_HASH_EVIDENCE_FILE="$evidence" \
      ...
      >"$run_dir/minio-hash-500.txt" 2>&1
    rc=$?
    set -e
    checked=0
    mismatches=0
    ...
    if (( rc == 0 && checked == sample_size && mismatches == 0 )); then
      ...
      checkpoint minio-hash-500-failed
      cat "$run_dir/minio-hash-500.txt" >&2
      exit 1
    fi
    ;;
