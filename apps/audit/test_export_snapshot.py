...
class AuditLogExportSnapshotTests(TestCase):
    def setUp(self):
        self.officer = make_user(personnel_number="0490", role=User.Role.SECURITY_OFFICER)
        self.client = Client()
        self.client.login(personnel_number=self.officer.personnel_number, password=PASSWORD)
