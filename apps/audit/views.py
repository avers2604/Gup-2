...
def filtered_entries(form):
    ...
    Для валидной формы экран и выгрузка используют одну и ту же выборку.
    Невалидный bound-ввод обрабатывается fail-closed: экран показывает ошибки
    формы без строк журнала, а экспорт отклоняет запрос кодом 400.
    """
