...
class AuditLog(models.Model):
    ...
    class EventType(models.TextChoices):
        ...
        # Выдача ссылки на файл документа с грифом ДСП
        # (apps/documents/file_views.py). Пишется ПЕРЕД тем, как ссылка
        # уходит пользователю: выдача ДСП без следа в журнале недопустима.
        # Для общедоступных документов не пишется намеренно — иначе записи,
        # ради которых журнал и заводился, утонут в шуме.
        ...
        # Выгрузка самого журнала аудита в CSV (apps/audit/views.py,
        # AuditLogExportView) — основа Grafana-алерта «экспорт журнала
        # аудита» (решение Заказчика). Запись создаётся ДО начала отдачи
        # потока: иначе оборванная выгрузка не оставила бы следа. В details
        # попадают применённые фильтры и число совпавших записей, но не сами
        # записи.
