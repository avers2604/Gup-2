...
INHERITED_PAIRS = (
    ...
    # Шапка приложения. Фон задаёт .app-header, цвет — правило на потомке,
    # поэтому в одном правиле пара не встречается и сверка по CSS её не
    # находит. Пропущенный случай стоил нечитаемого ФИО (контраст 1.94).
    ("подпись пользователя на шапке", "--text-muted-on-navy", "--color-navy"),
    ("пункт меню на шапке", "--text-on-navy", "--color-navy"),
)
