...
INHERITED_PAIRS = (
    ...
    # Боковое меню. Фон задаёт .sidebar градиентом, цвет — правило на
    # потомке, поэтому в одном правиле пара не встречается и сверка по
    # CSS её не находит. Пропущенный случай уже стоил нечитаемого ФИО в
    # прежней горизонтальной шапке (контраст 1.94).
    #
    # Проверяется по тёмному концу градиента (--color-navy-deep): он
    # худший для обводки значков и лучший для белого текста, а брать
    # надо тот конец, на котором пара слабее. Для светлого текста это
    # верхний конец, --color-navy, — он тоже здесь.
    ("пункт меню на боковом меню (верх)", "--text-on-navy", "--color-navy"),
    ("пункт меню на боковом меню (низ)", "--text-on-navy", "--color-navy-deep"),
    ("подвал бокового меню (верх)", "--text-muted-on-navy", "--color-navy"),
    ("подвал бокового меню (низ)", "--text-muted-on-navy", "--color-navy-deep"),
    # Верхняя панель — обычная светлая поверхность, но ФИО и должность в
    # ней набраны приглушённым, и пара так же распадается по двум
    # правилам (.topbar задаёт фон, .user-chip__role — цвет).
    ("должность в верхней панели", "--text-muted", "--surface"),
)
