...
class OcrTriggerOnSaveTests(TestCase):
    def test_creating_document_with_file_enqueues_ocr(self):
        with patch("apps.documents.tasks.run_ocr_for_document.apply_async") as mock_delay:
            with self.captureOnCommitCallbacks(execute=True):
                doc = make_document(
                    reg_number="TRIG-1", files_original="documents/originals/2026/01/trig-1.pdf",
                )
        mock_delay.assert_called_once_with(args=[str(doc.pk)], task_id=ANY, retry=False)
